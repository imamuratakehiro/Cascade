...
class JNet128Embnet(nn.Module):
    def __init__(self, cfg, inst_list, f_size, mono=True, to1d_mode="mean_linear", order="timefreq", mel=False, n_mels=259):
        ...
        if cfg.self_att:
            self.self_att = nn.MultiheadAttention(embed_dim=in_channel, num_heads=1, batch_first=True)
        if len(self.cfg.inst_list) == 1:
            self.embnet = To1D128(to1d_mode=to1d_mode, order=order, in_channel=in_channel)
        else:
            self.embnet = To1D640(to1d_mode=to1d_mode, order=order, in_channel=in_channel)
    ...
    def forward(self, input):
        ...
        #print(size)
        if self.cfg.self_att:
            B, C, F, T = conv6_out.shape
            q = k = v = conv6_out.reshape(B, C * F, T).permute(0, 2, 1)
            out, _ = self.self_att(q, k, v)
            conv6_out = out.permute(0, 2, 1).reshape(B, C, F, T)
        ...
        if len(self.cfg.inst_list) == 1:
            output_probability = {inst: torch.log(torch.sqrt(torch.sum(emb**2, dim=1))) for inst in self.cfg.inst_list}
        else:
