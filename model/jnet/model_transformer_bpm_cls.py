...
class AddPositionalEncodingBasedBPM(nn.Module):
    ...
    def forward(self, x: torch.Tensor, bpm, inst) -> torch.Tensor:
        ...
        for b in range(B):
            ...
            if inst == "mix":
                beat_len = math.ceil(60 / bpm[b, 0] * 44100 / self.cfg.hop_length)
            else:
                beat_len = math.ceil(60 / bpm[b, self.cfg.inst_all.index(inst)] * 44100 / self.cfg.hop_length)
...
class TransformerWithBPMCLS(nn.Module):
    def __init__(self, cfg, inst_list, f_size, mono=True, to1d_mode="mean_linear", order="timefreq", mel=False, n_mels=259):
        ...
        self.class_token = nn.Parameter(torch.randn(1, 1, in_channel))
    ...
    def forward(self, input, bpm, inst):
        ...
        if self.cfg.add_bpm:
            ...
            out_att = torch.concat([x[:, 0], self.normalize_bpm(bpm)], dim=1)
        output_emb = self.mlp(out_att) # encoderのclass token部分の出力
